    ...
    fn handle(p: ::UpdateParameters, raw_request: Vec<u8>) -> Result<json::JsonValue, IPCError>{
        ...
        let (function_name, parameters) = match json_request {
            json::JsonValue::Object(ref obj) => {
                let function_name = obj.get("function").and_then(|o| o.as_str()).ok_or(IPCError::new("Missing function name", json_request.dump()))?;
...
fn start_connection(mut connection: UnixStream, request_sink: chan::Sender<IPCRequest>) {
    let _ = thread::Builder::new().name("IPC Connection".to_owned()).spawn(move || {
...
impl IPC {
    pub fn setup() -> ::std::io::Result<Self> {
        ...
        let runtime_dir = ::std::env::var_os("XDG_RUNTIME_DIR").unwrap_or(OsString::from(FALLBACK_RUNTIME_DIR));
        ...
        use rand::Rng;
        let socket_name = ::rand::thread_rng().gen_ascii_chars().take(SOCKET_IDENTIFIER_LENGTH).collect::<String>();
        let socket_path = ugdb_dir.join(socket_name);

        let listener = UnixListener::bind(&socket_path)?;

        let _ = thread::Builder::new().name("IPC Connection Listener".to_owned()).spawn(move || {
