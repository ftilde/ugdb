            ...
            let (output, new_state) = match self.state {
                ...
                LexerState::PendingOutput(output) => {
                    match c {
                        ...
                        '{' => (Some(output), LexerState::PendingOutput((i, Token::LBrace, i+1))),
                        '}' => (Some(output), LexerState::PendingOutput((i, Token::RBrace, i+1))),
                        '[' => (Some(output), LexerState::PendingOutput((i, Token::LSquareBracket, i+1))),
                        ']' => (Some(output), LexerState::PendingOutput((i, Token::RSquareBracket, i+1))),
                        ',' => (Some(output), LexerState::PendingOutput((i, Token::Comma, i+1))),
                        '=' => (Some(output), LexerState::PendingOutput((i, Token::Equals, i+1))),
                ...
                LexerState::InStringEscapedChar(begin) => {
                    match c {
                        _ => (None, LexerState::InString(begin)),
                    }
                },
                LexerState::InText(begin, end) => {
                    match c {
                        ...
                        '{' => (Some((begin, Token::Text, end)), LexerState::PendingOutput((i, Token::LBrace, i+1))),
                        '}' => (Some((begin, Token::Text, end)), LexerState::PendingOutput((i, Token::RBrace, i+1))),
                        '[' => (Some((begin, Token::Text, end)), LexerState::PendingOutput((i, Token::LSquareBracket, i+1))),
                        ']' => (Some((begin, Token::Text, end)), LexerState::PendingOutput((i, Token::RSquareBracket, i+1))),
...
mod test {
    ...
    fn assert_eq_lexer_tokens(s: &'static str, expected_tokens: &[Token]) {
        let tokens = Lexer::new(s).map(|t| t.map(|val| val.1)).collect::<Result<Vec<_>, LexicalError>>().unwrap();
    ...
    fn test_lexer_basic_success() {
        ...
        assert_eq_lexer_tokens("lj \"dlfj}[{}]=,  \\t \\\"\"    dfdf sadfad {{  []},   =\t\t", &[Token::Text, Token::String, Token::Text, Token::LBrace, Token::LBrace, Token::LSquareBracket, Token::RSquareBracket, Token::RBrace, Token::Comma, Token::Equals]);
    }

    fn assert_eq_lexer_error(s: &'static str, expected_error: LexicalError) {
        match Lexer::new(s).map(|t| t.map(|val| val.1)).collect::<Result<Vec<_>, LexicalError>>() {
    ...
    fn test_lexer_error() {
        ...
        assert_eq_lexer_error("\"{,..\"\"", LexicalError::UnfinishedString{ begin_index: 6 });
    }
}
